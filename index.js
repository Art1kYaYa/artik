...
// Хранилище для отслеживания попыток подтверждения и ошибок
...
bot.onText(/\/оформить_доставку/, (msg) => {
  ...
  if (deliveryConfirmations[chatId]) {
    bot.sendMessage(
      chatId,
      `Вы подтвердили намерение оформить доставку. Теперь заполните анкету по образцу:\n\n` +
        `*Образец:*\n\n` +
        `*Никнейм:* Ваш_Никнейм\n` +
        `Товары: Ваши_Товары\n` +
        `*Координаты:* x y z\n` +
  ...
  deliveryConfirmations[chatId] = true;
  bot.sendMessage(chatId, 'Вы уверены, что хотите оформить доставку? Введите команду /оформить_доставку еще раз для подтверждения.');
});

// Проверка формата заявки на доставку
bot.on('message', (msg) => {
  const chatId = msg.chat.id;
  const text = msg.text;

  const deliveryFormatRegex =
    /Никнейм: (.+)\nТовары: (.+)\nКоординаты: (.+)\nДата оформление доставки: (\d{2}\/\d{2}\/\d{4})/;
  ...
  if (deliveryFormatRegex.test(text)) {
    ...
    bot.sendMessage(
      ...
      `Новая заявка на доставку:\n\n` +
        ...
        `Дата: ${deliveryData.date}`,
      {
        reply_markup: {
          inline_keyboard: [
            [{ text: '✅ Подтвердить доставку', callback_data: `approve_delivery_${msg.message_id}` }],
          ],
        },
      }
    );

    bot.sendMessage(chatId, '✅ Ваша заявка на доставку успешно отправлена!');
  } else if (deliveryConfirmations[chatId]) {
  ...
  `❌ Сообщение не соответствует формату.\n\n` +
    ...
    `*Товары:* Ваши_Товары\n` +
    `*Координаты:* x y z\n` +
    `*Дата оформления доставки:* 25/11/2024\n\n` +
    `_*Сообщение должно строго соответствовать формату!*_`,
  { parse_mode: 'Markdown' }
);
    ...
    bot.sendMessage(chatId, '❌ Сообщение не соответствует формату. Пожалуйста, следуйте образцу.');
  }
});

// Команда /submit_case для суда
